        ...
        <GlassCard className="p-8" hover={false}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="/forgot-password" className="text-sm text-poof-violet hover:underline">
                Forgot password?
              </Link>
            </div>

            {error && <div className="text-red-400 text-sm animate-fade-up">{error}</div>}

            <Button
              type="submit"
              className={cn(
                'w-full btn-press transition-all duration-300',
                success ? 'bg-poof-mint hover:bg-poof-mint' : 'bg-poof-accent hover:bg-poof-accent/90',
              )}
              disabled={loading || success}
            >
              {loading ? (
